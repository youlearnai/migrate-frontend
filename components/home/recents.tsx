...
const Recents = () => {
  ...
  const contentElements = isLoading
    ...
    : history && history.content_history.length > 0
      ? history.content_history.map((content, index) => (
          <Link
            href={
              content.space
                ? `/learn/space/${content.space.id}/content/${content.content.content_id}`
                : `/learn/content/${content.content.content_id}`
            }
            key={index}
  ...
  if (history?.content_history.length === 0 || !user || loading) {
    return null;
  }
      ...
      <div className="mb-4">
        <div className="text-left w-full flex justify-between items-center">
          <div className="flex flex-col">
            <span className="text-base lg:text-lg">{t("recents")}</span>
          </div>
          <Link href="/history" className="self-center">
            <Button
              variant="outline"
              size="sm"
              className="text-xs sm:text-sm border-none font-normal hover:bg-transparent hover:text-primary/70"
            >
              <span>{t("viewAll")}</span>
            </Button>
          </Link>
